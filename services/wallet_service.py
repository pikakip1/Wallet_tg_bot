...
class WalletService:
    ...
    async def add_wallet(self, name_wallet: str, amount: int | float):
        async for session in async_db_manager.get_session():
            await self.wallet_repository.add_wallet(session, name_wallet, amount)
    ...
    async def get_statistic_categories(
        ...
        statistic_categories = await self.wallet_repository.get_category_statistics(session=session, wallet_name=wallet_name)
    ...
    async def get_last_records(self, session: AsyncSession, wallet_name: str, count_records: int = 10) -> list[str]:
        records = await self.wallet_repository.get_last_records(
            session=session, wallet_name=wallet_name, count_records=count_records
        )
    ...
    async def get_wallet_balance(self, wallet_name: str):
        async for session in async_db_manager.get_session():
            return await self.wallet_repository.get_wallet_amount(session=session, wallet_name=wallet_name)
